...
def obtainPrimersForMeaningfulDigestAnalysisPartTwo(chromosome_sequence, restriction_site_region_start_position, restriction_site_region_end_position, base_pair_position_before_digest_cut, degree_of_asymmetry, minimum_spacing_length, primer_search_space_length, primer_length, maximum_amplicon_length):
    space_to_the_left = restriction_site_region_start_position # Correct
    space_to_the_right = len(chromosome_sequence) - restriction_site_region_end_position - 1 # Correct

    is_left_minimum = min(space_to_the_left, space_to_the_right) == space_to_the_left

    primer_search_space_availability = maximum_amplicon_length - restriction_site_region_end_position + restriction_site_region_start_position - 1

    left_availability = int(primer_search_space_availability / 2)
    right_availability = primer_search_space_availability - left_availability

    left_excess = space_to_the_left - left_availability
    right_excess = space_to_the_right - right_availability

    if left_excess < 0 and right_excess + left_excess >= 0:
        left_availability = space_to_the_left
        right_availability -= left_excess
    elif left_excess + right_excess >= 0 and right_excess < 0:
        right_availability = space_to_the_right
        left_availability -= right_excess
    elif left_excess < 0 and right_excess < 0:
        left_availability = space_to_the_left
        right_availability = space_to_the_right

    left_primer_search_space_start_position = restriction_site_region_start_position - left_availability
    left_primer_search_space_end_position = restriction_site_region_start_position - 1

    right_primer_search_space_start_position = restriction_site_region_end_position + 1
    right_primer_search_space_end_position = restriction_site_region_end_position + right_availability

    amplicon = chromosome_sequence[left_primer_search_space_start_position: (right_primer_search_space_end_position + 1)]

    left_primer_search_space_end_position -= left_primer_search_space_start_position
    right_primer_search_space_start_position -= left_primer_search_space_start_position
    right_primer_search_space_end_position -= left_primer_search_space_start_position
    left_primer_search_space_start_position = 0

    primers = obtainPrimers(amplicon, left_primer_search_space_start_position, left_availability, right_primer_search_space_start_position, right_availability)
    ...
    for left_and_right_primer in primers:
        ...
        left_primer_start_position = left_primer_search_space_start_position + left_primer_results[2]
        right_primer_end_position = left_primer_search_space_start_position + right_primer_results[2]
